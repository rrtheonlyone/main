...
****
* All fields need to have at least a value. e.g. `n/` is not allowed.
...
****
* p/ will only match with phone numbers.
* n/ will only match with names.
* At least one of the optional fields must be provided.
...
Format: `/order find NAME [MORE NAMES] +

****
* The search is case insensitive. e.g `tom` will match `Tom`
* Multiple names must be seperated by a comma. e.g. `/order find john, hilary`.
...
* `/deliveryman find john` +
Returns `john` and `John Doe`
* `/order find john, alice, tom` +
Returns `john`, `alice`, `tom`
...
==== Adding an order to a route : `/plan`

Add an existing order to an existing route +
Format: `/plan` o/ORDER_INDEX r/ROUTE_INDEX
...
`/route` +
`/plan o/3 r/4` +
Adds the 3rd order to the 4th route.
...
* *Find delivery man* : `/deliveryman find [n/NAME] [p/PHONE]` +
...
* *Add order to route* : `/plan o/ORDER_INDEX r/ROUTE_INDEX` +
e.g. `/plan o/1 r/1`
