...
functionality is that it allows for managers to keep track of the number of orders they have from
customers and send out deliverymen to handle those orders. In addition, FoodZoom is able to provide insights and
give location information for the orders that it has. The application was built by 5 students.
...
* *Major enhancement:* Completely *Revamped* and *Designed* the User Interface (UI) for the application
(Pull Request
...
actions of the user e.g. the map/dashboard will dynamically update when orders are added or edited.
