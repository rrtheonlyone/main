...
Expected: Order on orderlist panel is marked as `completed` and deliveryman is updated.
...
Expected: Fail to mark order as `completed` because only `ongoing` status can be marked as `completed`.

.. Prerequisites: Have some `completed` orders.
... Test case: Mark a valid index as completed but would fail. +
Command: `/order done <order_index_of_completed_status>` +
Expected: Fail to mark order as `completed` because it is already `completed`.
