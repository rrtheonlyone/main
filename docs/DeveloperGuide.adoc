...
changes and renders the user interface for the dashboard screen. This is done by attaching
a listener to the Observable List of orders. The code snippet demonstrating this is
as follows:
...
The `Display` component as such controls the flow of the dashboard. It is responsible for
rendering the Map and Statistics components along with passing in the data that
they need.

The controller helps to process the list of Orders and pass it to the 2 components in a
specific format so that the data can be rendered. The next two subsections in 3.2 would
elaborate on how this data processing is done.
...
refers to a specific district in Singapore.
...
image::statisticsPanel.png[width="790"]
