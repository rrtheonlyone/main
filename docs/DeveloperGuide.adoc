...
==== Current Implementation - Overall Dashboard

The dashboard for this application involve the main screen that the user first sees when he or
she logs into the application. Refer to diagram below:

image::Ui.png[width="790"]

The 2 main components in the dashboard are the:

..   Dynamic Map showing location of pending orders
..   Statistics Panel giving insights for the data

However, before delving into the details for those specific components, it is
important to consider some of the implementation details common to both.

First and foremost, the data for both these components are handled by a higher
level component, `Display` . This component listens for changes and updates the relevant data
to be rendered in those specific components. This is done as follows:
...
The following activity diagram summarize what happens inside the
`Display` Component when a user executes a new command:

image::UIListenerActivityDiagram.png[width="800"]

In summary, the `UIManager` will help render these components. The
`Display` component will handle the entire dashboard area. It
will re-render the panels automatically once there are changes made.

However, how exactly is the data processed? The following two subsections
will shed further light on this.

==== Current Implementation - Map Component

The Map helps to render where all the pending orders are based on the
address of Order given. The map looks like this:

image::Ui.png[width="790"]

The most noteworthy feature behind the map is that it works offline. It
does not make an external call to Google Maps API or other external
services. This section will illustrate how this is done.

The map is designed to give a *reasonable* approximation of the order
based on the postal code that has been provided. The first 2 digits of
a Singapore postal code refers to a specific district in Singapore. The
image below shows how it is demarcated.

image::SingaporeDemarcation.png[width="790"]

For example, if the first 2 digits of the given postal code is "61", the
Order falls under district 22 (with reference to the image above).

Based on this information in order to create an offline map, three
...
. A `MapPanel` component that renders placeholders on the grid or clears
placeholders if there has been a change

Using the above, components the higher level `Display` component is able
to pass in a HashMap (where the keys are the first 2 digits of the postal
code and the values is the count) to the MapPanel. The MapPanel then
renders the placeholders based on the count and postal code information
given.

The code snippet for initialisation the placeholders is given below:
```
...
==== Current Implementation - Statistics Panel

Another aspect for the dashboard is the statistics panel. An image
showing what is in the panel can be seen below.
...
For statistics (1) and (2), it is simply a matter of counting the total
orders and checking how many of them are pending. For statistics (3) and (4)
on the other hand, some considerations had to be done on the choice of
data structure.

In order to get the most common food item, a hashtable was used. The keys
were the food items and the value would be the number of times this item
appears. Every time, the list was updated, this hashtable would be updated
and the maximum returned.

For order history however, using a hashtable would not be efficient; it is
computationally expensive to get the 7 most recent dates in which they was
at least one order. A more efficient solution involved the use of a
balanced binary search tree (or Java `TreeMap`). Using a custom comparator,
all that had to be done was check for the top 7 entries in the tree.
...
** Pros:
- Since the listener is attached in the UI component itself, it is
more convenient for the functions to process the data be available in
the UI itself.
...
one consideration was to use a statistics manager under model. This
manager will take in a list of orders and return the required data.

** Pros:
- This solution is more scalable. By having a separate class dedicated to managing the
statistics of orders, it is easier to integrate much more
complex statistics for the application
** Cons:
